...
export default function Navbar() {
  ...
  // On home: transparent over hero, then solid on scroll
  // On other pages: solid from start
    ...
    <>
      <header
        className={`fixed top-0 left-0 right-0 z-50 transition-all duration-700 ${
          isTransparent
            ? 'bg-transparent'
            : 'bg-ivory/95 backdrop-blur-md border-b border-border-custom/50 shadow-sm'
        }`}
      >
        <nav className="max-w-[1400px] mx-auto px-6 sm:px-8 lg:px-12 h-[72px] sm:h-[80px] flex items-center justify-between">
          ...
          <Link href="/" className="flex items-center gap-3 group">
            <div className={`w-8 h-8 rounded-full border flex items-center justify-center transition-all duration-500 ${
              isTransparent
                ...
                : 'border-sage group-hover:bg-sage'
            }`}>
              <span className={`font-display text-sm transition-colors duration-500 ${
                isTransparent
            ...
            <div className="flex items-baseline gap-1.5">
              <span className={`font-display text-lg tracking-tight transition-colors duration-500 ${
                isTransparent ? 'text-white' : 'text-charcoal'
              }`}>
                MedVista
              </span>
              <span className={`text-[8px] tracking-[0.3em] uppercase font-body font-medium transition-colors duration-500 ${
                isTransparent ? 'text-white/40' : 'text-warm-gray'
              }`}>
                Premier
              </span>
            </div>
              ...
              return (
                <Link
                  ...
                  className={`relative font-body text-[13px] tracking-wide transition-colors duration-300 py-1 ${
                    isActive
                      ? isTransparent ? 'text-white' : 'text-charcoal'
                      : isTransparent ? 'text-white/50 hover:text-white/80' : 'text-warm-gray hover:text-charcoal'
                  }`}
                >
                  {link.label}
                  {isActive && (
                    <motion.div
                      ...
                      className={`absolute -bottom-1.5 left-1/2 -translate-x-1/2 w-1 h-1 rounded-full ${
                        isTransparent ? 'bg-sage-light' : 'bg-sage'
                      }`}
          ...
          <div className="flex items-center gap-3">
            <Link
              href="/contact"
              className={`hidden sm:inline-flex items-center gap-2 px-5 py-2.5 font-body text-[13px] rounded-full transition-all duration-300 ${
                isTransparent
