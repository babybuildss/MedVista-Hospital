...
export default function DepartmentsSection() {
  ...
  return (
    <section id="departments" className="relative py-20 md:py-28 bg-section-dark">
          ...
          <div className="max-w-md mx-auto mb-12">
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-12 pr-4 py-3 rounded-full bg-white/5 border border-white/10 text-white placeholder-gray-500 focus:border-[#d4a853] focus:ring-0 transition-colors"
              />
            </div>
          </div>
        </ScrollReveal>
            ...
            {filtered.map((dept, i) => (
              <ScrollReveal key={dept.name} delay={i * 0.05}>
                <motion.div
                  ...
                  className="group glass rounded-2xl p-6 cursor-pointer card-hover relative overflow-hidden"
                >
                  {/* Background Glow */}
                  <div
                    className={`absolute inset-0 bg-gradient-to-br ${dept.color} opacity-0 group-hover:opacity-10 transition-opacity duration-500 rounded-2xl`}
                  />

                  <div className="relative z-10">
                    <div
                      className={`w-14 h-14 rounded-xl bg-gradient-to-br ${dept.color} flex items-center justify-center mb-4 group-hover:scale-110 transition-transform duration-300`}
                    >
